...
notation:50 f:80 " é contínuo em " x₀ => continuous_function_at f x₀
notation:50 u:80 " converge para " l => sequence_tendsto u l

def increasing_seq (u : ℕ → ℝ) := ∀ n m, n ≤ m → u n ≤ u m

notation u " é crescente em" => increasing_seq u

def is_supremum (M : ℝ) (u : ℕ → ℝ) :=
(∀ n, u n ≤ M) ∧ ∀ ε > 0, ∃ n₀, u n₀ ≥ M - ε

notation M " é supremum de " u => is_supremum M u
...
protected def Verbose.Portuguese.Subset (s₁ s₂ : Set α) :=
  ∀ x, x ∈ s₁ → x ∈ s₂

instance (priority := high) Verbose.English.hasSubset : HasSubset (Set α) :=
  ⟨Verbose.Portuguese.Subset⟩

end Subset

open Verbose.Portuguese
